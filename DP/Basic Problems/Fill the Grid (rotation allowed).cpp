 ...
 *gievn, 2x1 and 2x2 tiles, and rotation allowed  
 *N<=100000
 */
...
int go(int n){
    ...
    if(n==2)return 3; //2x2 tiles can fill only in 3 ways
    //memoization
    if(dp[n]!=-1)return dp[n];

    //transition
                 //2x1 and 1x2 moves and 2x2  
    return dp[n] = go(n-1) + go(n-2) + go(n-2);
}
void task(){
    int n; cin >> n;
